...
// UPI Reserve Pay mandates. Authorising a block is the one step that needs a human — everything
// downstream (checkout, and the chat/agent callers coming next) debits headlessly against the
// token this produces.
...
// The caller's active mandate, or null. What a checkout screen asks before offering Reserve Pay.
// Must stay registered above GET /mandates/:id — Hono matches in declaration order, and the
...
// it the endpoint to poll while the customer approves the block in their UPI app.
...
// them back instantly, and marks it revoked locally so nothing debits it again.
...
// Test harness: charges the caller's own mandate without producing an order, so the rail can be
// exercised end to end before the cart path is involved. Real purchases go through
// POST /api/cart/checkout/reserve-pay, which creates an order row for the money it moves.
